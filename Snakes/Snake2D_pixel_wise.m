function [P,J]=Snake2D_pixel_wise(I,P,Options)
...
defaultoptions=struct('Verbose',false,'nPoints',100,'Wline',0.04,'Wedge',2,...
    'Wterm',0.01,'Sigma1',10,'Sigma2',20,'Alpha',0.2,'Beta',0.2,'Delta',0.1,...
...
Eext = ExternalForceImage2D(I,Options.Wline, Options.Wedge, Options.Wterm, Options.Sigma1, 0);
...
% smooth contour
if isscalar(Options.Beta)
    beta=Options.Beta;
    Options.Beta=beta*ones(size(I));
    if Options.Verbose
        disp('Scalar beta')
    end
else
    if Options.Verbose
        disp('Pixel-wise beta')
    end
end
[A,B]=SnakeInternalForceMatrix2D_pixel_wise(P,Options.nPoints,Options.Alpha,Options.Beta,Options.Gamma);
h=[];
for i=1:Options.Iterations
%     display(['Iter ' num2str(i)])
    P0=P;
    P=SnakeMoveIteration2D_pixel_wise(I,A,B,P,Fext,Options.Gamma,Options.Kappa,Options.deltaMatFlag,Options.Delta,Options.deltaMat,...
        Options.RVEFlag,Options.RVECoeff,Options.ShapeSimFlag,Options.ShapeSimRef,Options.ShapeSimCoeffs);
    
    % add break condition to prevent snake moves out of the image
    if any(any(isnan(P)))
        P=P0;
        break
    end
...
if(Options.Verbose)
    subplot(2,2,4)
    imshow(I), hold on;
    plot([P_init(:,2);P_init(1,2)],[P_init(:,1);P_init(1,1)],'b.','LineWidth',2);
    plot([P(:,2);P(1,2)],[P(:,1);P(1,1)],'r.-','LineWidth',2);
